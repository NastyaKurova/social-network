...
export const Pagination: FC<PaginationPropsType> = ({
    ...
    return (
        ...
        <div>
            {portionNumber > 1 &&
                <span className={styles.pagination} onClick={() => setPortionNumber(portionNumber - 1)}>{'<'}</span>}

            {pages.filter(p => p >= portionNumberLeftNumber && p <= portionNumberRightNumber).map((page, index) =>
                <span
